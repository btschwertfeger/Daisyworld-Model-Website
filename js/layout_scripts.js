...
RESET_BTN.onclick = function () {
  ...
  DE_SLIDERS.forEach((element, index) => {
    ...
    document.getElementById(dw_variabels[index] + "_sliderAmount").innerHTML =
      default_value;
  });
};
...
for (let entry = 0; entry < DE_SLIDERS.length; entry++) {
  DE_SLIDERS[entry].oninput = function () {
    document.getElementById(DE_SLIDERS[entry].id + "rAmount").innerHTML =
      document.getElementById(DE_SLIDERS[entry].id).value;
  };
