...
window.areaPlot_checkboxes = [
  "showBlackDinc-checkbox",
  "showWhiteDinc-checkbox",
  "showBlackDdec-checkbox",
  "showWhiteDdec-checkbox",
  "showTotalAmountD4incL-checkbox",
  "showTotalAmountD4decL-checkbox",
];
window.tempPlot_checkboxes = [
  "showglobTinc-checkbox",
  "showglobTdec-checkbox",
...
window.createPlots = function (input = window.default_parameters) {
  ...
  let black_data_inc = new Array(
      window.lastResult["area_black_vec_inc"].length,
    ),
  ...
  for (let entry = 0; entry < black_data_inc.length; entry++) {
    black_data_inc[entry] =
      window.lastResult["area_black_vec_inc"][entry] * 100;
    white_data_inc[entry] =
      window.lastResult["area_white_vec_inc"][entry] * 100;
    black_data_dec[entry] =
      window.lastResult["area_black_vec_dec"][entry] * 100;
    white_data_dec[entry] =
      window.lastResult["area_white_vec_dec"][entry] * 100;
  }
  ...
  window.areaPlot_checkboxes.forEach((element, index) => {
    ACTIVE_DATA_AREA_PLOT[index] = document.getElementById(element).checked
      ? window.ALL_AREA_DATASETS[index]
      : {};
  });

  document.getElementById("daisyworld-area-plot").remove();
  document.getElementById("daisyworld-area-plot-container").innerHTML =
    '<canvas id="daisyworld-area-plot"></canvas>';
  ...
  for (let entry = 0; entry < temp_lumi_data_inc.length; entry++) {
    temp_lumi_data_inc[entry] =
      window.lastResult["Tp_vec_inc"][entry] -
      window.lastResult["KELVIN_OFFSET"];
    temp_lumi_data_dec[entry] =
      window.lastResult["Tp_vec_dec"][entry] -
      window.lastResult["KELVIN_OFFSET"];
  }
  ...
  window.tempPlot_checkboxes.forEach((element, index) => {
    ...
    ACTIVE_DATA_TEMP_PLOT[index + 1] = document.getElementById(element).checked
      ? window.ALL_TEMP_DATASETS[index + 1]
      : {};
  });

  document.getElementById("daisyworld-temperature-plot").remove();
  document.getElementById("daisyworld-temperature-plot-container").innerHTML =
    '<canvas id="daisyworld-temperature-plot"></canvas>';
...
window.updatePlot = (what) => {
  if (what == "area") {
    ...
    window.tempPlot_checkboxes.forEach((element, index) => {
      // index+1 because the optimal temperature is alwys shown
      ACTIVE_DATA_TEMP_PLOT[index + 1] = document.getElementById(element)
        .checked
        ? window.ALL_TEMP_DATASETS[index + 1]
        : {};
    });
...
function computeDaisyworld(input, kind) {
  ...
  fluxes.forEach(function (flux, index) {
    ...
    temp_without_life_vec[index] =
      Math.pow(((So * flux) / sigma) * (1 - alb_barren * p), 1 / 4) -
      KELVIN_OFFSET;
    Tp_vec[index] = Tp;
  });
  ...
  for (let entry = 0; entry < total_amount_daisies.length; entry++) {
    total_amount_daisies[entry] =
      (area_black_vec[entry] + area_white_vec[entry]) * 100;
  }
  if (kind == "incr") {
    return {
      fluxes_inc: fluxes,
